...
import type { PexelsPhoto, PexelsResponse, Category } from '@/types/pexels';
import { useState, useEffect, useCallback } from 'react';
import Image from 'next/image';
import Link from 'next/link';
import { Input } from '@/components/ui/input';
import { Button } from '@/components/ui/button';
import { Search, Download, X, Menu, Home as HomeIcon } from 'lucide-react';
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
import { useRouter, useSearchParams } from 'next/navigation'; // Keep for client-side navigation
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
import { wallpaperCategories } from '@/config/categories';
...
export default function SearchPageClient({ initialQuery, initialCategory }: SearchPageClientProps) {
  ...
  // useSearchParams can still be used if needed for parts of the component that react to URL changes after initial load
  ...
  useEffect(() => {
    // Update internal state if props change (e.g., user navigates directly to a different search URL)
  ...
  const handleSearchSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const newSearchTermInput = formData.get('search') as string;
    const trimmedSearchTerm = newSearchTermInput.trim();
    const effectiveSearchTerm = trimmedSearchTerm || 'Wallpaper';

    // Navigate using the router. The component's state `currentCategory` is used.
  ...
  const handleDeviceCategoryChange = (newCategory: Category) => {
    if (newCategory !== currentCategory) {
      // Navigate using the router. The component's state `searchTerm` is used.
  ...
  const handleDownload = async () => {
    ...
    toast({ title: "Download Starting", description: `Preparing ${filename} for download...` });
    try {
      ...
      toast({ title: "Download Failed", description: "Could not download the wallpaper. Please try again.", variant: "destructive" });
    }
  };
  ...
  const modalAspectRatio = selectedWallpaper
    ? selectedWallpaper.width / selectedWallpaper.height > 1.2 ? 'aspect-video' : 'aspect-[9/16]'
    : gridAspectRatio;
      ...
      <div className="flex flex-col min-h-screen bg-background text-foreground">
         <header className="sticky top-0 z-20 bg-background/90 backdrop-blur-sm border-b border-border">
          <div className="container mx-auto max-w-7xl px-3 sm:px-4 py-2.5 flex flex-col sm:flex-row items-center justify-between gap-2 sm:gap-3">
              <Link href="/" className="text-xl sm:text-2xl font-bold text-primary self-center sm:self-auto flex items-center gap-1.5 sm:gap-2">
                <HomeIcon className="h-5 w-5 sm:h-6 sm:w-6" /> Wallify
              </Link>
              ...
              <form onSubmit={handleSearchSubmit} className="flex gap-2 items-center w-full sm:w-auto sm:flex-grow max-w-xs sm:max-w-sm md:max-w-md">
                  <div className="relative flex-grow">
                      ...
                      <Input
                          ...
                          defaultValue={searchTerm} // Use state `searchTerm`
                          key={searchTerm} // Re-initialize if searchTerm state changes from outside (e.g. prop change)
            ...
            <>
              ...
              {hasMore && !loading && (
                <div className="flex justify-center mt-6 sm:mt-8 mb-4">
                  <Button onClick={handleLoadMore} variant="outline" size="lg" className="text-sm px-6 py-2.5">Load More</Button>
                </div>
              )}
