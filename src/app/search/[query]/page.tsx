...
export default function SearchPage({ params }: SearchPageProps) {
  ...
  const handleSearchSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (effectiveSearchTerm !== searchTerm || currentCategory !== (searchParams.get('category') as Category)) {
  ...
  const modalAspectRatio = selectedWallpaper
    ? selectedWallpaper.width / selectedWallpaper.height > 1 ? 'aspect-video' : 'aspect-[9/16]'
    : gridAspectRatio;
      ...
      <div className="flex flex-col min-h-screen bg-background text-foreground">
         <header className="sticky top-0 z-20 bg-background/90 backdrop-blur-sm border-b border-border">
          <div className="container mx-auto max-w-6xl px-4 py-3 flex flex-col sm:flex-row items-center justify-between gap-3">
              <Link href="/" className="text-2xl sm:text-3xl font-bold text-primary self-center sm:self-auto flex items-center gap-2">
                <HomeIcon className="h-6 w-6 sm:h-7 sm:w-7" /> Wallify
              </Link>
            ...
            <>
              ...
              {hasMore && !loading && (
                <div className="flex justify-center mt-8 mb-4">
                  <Button onClick={handleLoadMore} variant="outline" size="lg">Load More</Button>
                </div>
              )}
